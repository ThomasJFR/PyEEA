...
class LearningCurve(Annuity):
        ...
        Args:
            first_amount [float]
        """
        def __init__(self, first_amount, learning_rate, d, final_amount=None, title=None, tags=None):
            super().__init__(first_amount, d, title, tags)
            self.learning_rate = float(learning_rate)
            self.b = log(self.learning_rate) / log(2.)
            ...
            for n in ns:
                if self.d[0] < n <= self.d[1]:
                    ...
                    if self.final_amount is not None:
                        dirn = copysign(1., self.final_amount - self.amount)
