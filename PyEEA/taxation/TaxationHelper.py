...
class Tax:
    ...
    def generate_cashflow(self, cashflows=[], depreciations=[]):
        ...
        taxable_cashflows = [cf for cf in cashflows if self._tag in cf.tags] or [NullCashflow()]
        shielding_depreciations = [dp for dp in depreciations if self._tag in dp.tags] or [NullCashflow()]
        
        # Create and return the TaxCashflow object
        return TaxCashflow(
            self._rate, taxable_cashflows, shielding_depreciations, 
            get_final_period(cashflows, finite=True), title=self.get_title(), tags=self._tag)
...
class TaxCashflow(Dynamic):
    ...
    def tax_fun(self, n):
        ...
        shielding_sum = sum([
            depreciation[n]
            for depreciation in self._depreciations
        ])
